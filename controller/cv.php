...
switch ($act) {
    ...
    if (
        ...
        // Không lọc -> hiện CV nổi bật
        $list_cv = list_cv_featured();
    ...
    case 'manage_Cv':
        ...
        $list_saved_recr = list_saved_recr($iduser);
        ...
        break;
function get_cv_list($is_featured = false) {
    if ($is_featured) {
        $list = load_all_cv_by_score(); // Hàm này cần định nghĩa trong model
        return array_filter($list, function($cv) {
            return isset($cv['completion_score']) && $cv['completion_score'] >= 3;
        });
    }
    return load_all_cv();
}
