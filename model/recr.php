...
function recr_add(
    ...
    $sql = "INSERT INTO recr( idcorp, job, exp, level, salary,
         progLang, type, description, start, end, request,status) 
...
function recr_select_all($kym, $level, $typeRecr, $salary, $progLang, $address, $exp , $job)
{
    ...
    if ($kym != "") {
        $sql .= " AND (
                    ...
                    OR r.exp LIKE '%" . $kym . "%'
                    OR u.name LIKE '%" . $kym . "%'
                    OR r.progLang LIKE '%" . $kym . "%'
                    OR u.address LIKE '%" . $kym . "%'
                )";
    }
...
{
    $sql = "SELECT r.* , c.introduce , c.benefits, u.address , u.name , u.phone , u.email , g.avatar 
            FROM recr r 
            LEFT JOIN corp c ON c.id = r.idcorp  
            LEFT JOIN user u ON u.id = c.iduser 
            LEFT JOIN gallery g ON u.id = g.iduser 
...
{
    $sql = "SELECT r.* , c.introduce , c.benefits, u.address , u.name , g.avatar
            FROM recr r
            LEFT JOIN corp c ON c.id = r.idcorp
            LEFT JOIN user u ON u.id = c.iduser
            LEFT JOIN gallery g ON u.id = g.iduser
...
function save_recr($iduser, $idrecr)
{
    $sql = "SELECT * FROM saved_recr WHERE iduser = ? AND idrecr = ?";
    $check = pdo_query_one($sql, $iduser, $idrecr);

    if (!$check) {
        $sql = "INSERT INTO saved_recr(iduser, idrecr) VALUES(?, ?)";
        pdo_execute($sql, $iduser, $idrecr);
    }
}

function list_saved_recr($iduser)
{
    $sql = "SELECT 
                r.id AS idrecr,
                r.job AS job,
                r.level AS level,
                r.type AS type,
                r.progLang AS progLang,
                r.start AS start,
                r.salary AS salary,

                u.name AS companyName,
                u.address AS companyAddress,
                g.avatar AS companyAvatar,

                s.id AS idsaved
            FROM saved_recr s
            INNER JOIN recr r ON s.idrecr = r.id
...
function check_saved_recr($iduser, $idrecr)
{
    $sql = "SELECT * FROM saved_recr 
            WHERE iduser = ? AND idrecr = ?";

    return pdo_query_one($sql, $iduser, $idrecr);
}
function unsave_recr($iduser, $idrecr)
{
    $sql = "DELETE FROM saved_recr
            WHERE iduser = ? AND idrecr = ?";
