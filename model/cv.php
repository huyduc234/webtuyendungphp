...
function list_cv_featured()
{
    $sql = "SELECT 
                ...
                u.address,
                g.avatar,
                COUNT(DISTINCT s.id) AS totalSkill,
                COUNT(DISTINCT e.id) AS totalExp,
                COUNT(DISTINCT d.id) AS totalDegree
            FROM cv
            INNER JOIN user u ON cv.iduser = u.id
            LEFT JOIN gallery g ON u.id = g.iduser
            LEFT JOIN skillcv s ON cv.id = s.idcv AND s.progLang != ''
            LEFT JOIN expcv e ON cv.id = e.idcv AND e.job != ''
            LEFT JOIN degree d ON cv.id = d.idcv AND d.name != ''
            WHERE u.role = 2
              AND cv.major != ''
              AND cv.salary != ''
              AND cv.introduce != ''
            GROUP BY cv.id
            HAVING totalSkill > 0 OR totalExp > 0 OR totalDegree > 0
            ORDER BY 
                totalSkill DESC,
                totalExp DESC,
                totalDegree DESC,
