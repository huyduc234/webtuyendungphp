...
function update_recr_status($id, $status)
{
    $sql = "UPDATE recr SET status = ? WHERE id = ?";
    pdo_execute($sql, $status, $id);
}
function countRecrStatus($status)
{
    $sql = "SELECT COUNT(*) AS total FROM recr WHERE status = ?";
    $result = pdo_query_one($sql, $status);
    return $result['total'];
}